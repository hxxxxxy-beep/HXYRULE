...
- [ ] Native site heart (add/remove favourite AJAX) patches the Favorites or playlist index automatically (same as extension Edit). After a sex baseline exists, small adds auto-tag Futa/Straight via detail-page futanari checks; larger batches start a visible continue-remaining sex job (or status says incomplete → **Retag sex**).
- [ ] **Local / Not local** dual toggles after Scan (both on = all; Local only / Not local only work).
- [ ] **Futa / Straight** dual toggles follow video detail tags: **Tag sex** fetches each video page and labels **Futa** when the `futanari` tag is present, otherwise **Straight**. Progress shows on the **Tag sex** button (`Tagging (n/total)` or `Tagging remaining (n/total)` while continuing); Compact / Show matches keep their own labels (not overwritten by Tag sex). UI never shows the word “delta”.
- [ ] **Tag sex** Stop / incomplete: pause stores an id-set fingerprint; next **Retag sex** compares the live index — unchanged → continue untagged only; changed → still continue on current untagged (new adds included, removed ids ignored). Site 429 cools and retries in-job (does not abandon the remainder after a few waves).
- [ ] **Retag sex** with a complete baseline (or corrupt labels) shows a modern danger confirm: title **Retag all videos?**, body *Clears all Futa/Straight labels and rebuilds them from each video page.*, actions Cancel / **Retag all**. Continuing remaining tags does not confirm. First-time Tag sex (no labels yet) does not confirm.
...
- [ ] Index/Renumber/Tag buttons show progress (`Indexing 3/100` or `Tagging (12/100)` / `Tagging remaining (12/100)`) and keep final counts (`Rebuild index (100/100)` or `Build index (100/100)`). Sex idle label is **Tag sex** with no baseline, **Retag sex** once any sex labels exist.
